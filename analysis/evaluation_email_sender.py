...
def build_email_body(summary):
    ...
    if correction:
        ...
        lines += [
            ...
            "## 4. 纠偏效果",
            "",
            f"  - 来源记录: {correction.get('source_rows', cs.get('total', 0))}",
            f"  - 有效股票样本: {correction.get('eligible_rows', cs.get('total', 0))}",
            f"  - 策略明细行: {correction.get('eligible_strategy_rows', cs.get('total', 0))}",
            f"  - 排除旧格式记录: {correction.get('excluded_legacy_rows', 0)}",
            f"  - 被纠偏降级: {cs.get('downgraded', 0)}",
            f"  - 避坑: {cs.get('pit_avoided', 0)}",
            f"  - 误杀: {cs.get('false_negative', 0)}",
            f"  - 避坑率: {_pct(cs.get('pit_avoid_rate'))}",
            f"  - 误杀率: {_pct(cs.get('false_negative_rate'))}",
