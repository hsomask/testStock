...
from analysis import board_classifier
...
def classify_concept_label(name):
    ...
    return board_classifier.classify_concept_label(name)


def is_industrial_theme(name):
    """是否为产业概念（可进入主线）"""
    return board_classifier.is_mainline_board(name)
...
def label_category_name(cat):
    """分类标签 → 中文名"""
    return board_classifier.label_category_name(cat)
...
def filter_effective_themes(themes):
    ...
    for t in (themes or []):
        name = t.get("name", "")
        info = board_classifier.classify_board(name)
        if info.category != "industrial":
            ...
            old = effective_by_cluster.get(cluster)
            if old is None or candidate.get("score", 0) > old.get("score", 0):
                effective_by_cluster[cluster] = candidate
    return list(effective_by_cluster.values()), dynamic
...
def explain_non_industrial_label(name, category):
    """按标签类型生成个性化解释"""
    return board_classifier.explain_non_industrial_label(name, category)
